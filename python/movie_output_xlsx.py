            ...
            if counter == 2000:
                ...
                if opWrapper.waitAndPop(datumProcessed):
                    if not args[0].no_display:
                        ...
                        for d in range(len(datum.poseKeypoints)): #識別されている人数だけ回す
                            ...
                            if sekitui != 0:
                                ...
                                if end_y - start_y < 1 or end_x - start_x < 1:
                                    ...
                                    person_image = imageToProcess[start_y: end_y, start_x: end_x]
                                    ...
                                    for i, val in enumerate(keypoint_judge.values()): #keypointの数だけ回す。余裕があったらkeypoint_judgeの中で1になっている数だけ回す。そのindexを取得するようにしたら、1行下のif文がいらなくなる。
                                        if val == 1: #keypointを使うかどうか
                                            ...
                                            if(prob == 0):
                                                ...
                                                # start_y = keypoint_y - height_r if keypoint_y - height_r  > 0 else 0
                                                # end_y = keypoint_y + height_r if keypoint_y + height_r < img_resize.shape[1]  else img_resize.shape[1]
                                                # start_x =  keypoint_x - width_r if keypoint_x - width_r  > 0 else 0
                                                # end_x = keypoint_x + width_r if keypoint_x + width_r  < img_resize.shape[0] else img_resize.shape[0]
                                                # print(img_resize.shape)
                                                # img_area = person_image[start_y: end_y, start_x: end_x]
                        ...
                        cv2.imwrite('../examples/person_cut/' + file_path[fil] + '_' + str(counter) + '_' +  '.png', datum.cvOutputData)
